...
MMD自動トレースの足FKを足IKに変換する時には、「足首水平化」「つま先・かかとY=0」オプションをONにして下さい。
...
・指定されたモーフを任意の条件に合わせて指定倍率で補正します。
・条件は「より大きい(＞)」「以上(≧)」「等しい(＝)」「以下(≦)」「より小さい(＜)」から選ぶ事ができます。


----------------
9) 捩りOFF

・モーション内のキーフレのうち、腕捩りを腕に、手捩りをひじに統合し、捩りキーを削除します。
・お借りしたモーションで捻じキーが必須条件となっている場合、捩りを再調整して付与してください。
