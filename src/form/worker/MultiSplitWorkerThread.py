...
class MultiSplitWorkerThread(BaseWorkerThread):
    ...
    def thread_event(self):
        try:
            ...
            self.result = self.frame.multi_split_panel_ctrl.vmd_file_ctrl.load() and self.result
            ...
            if self.result:
                self.options = MMultiSplitOptions(\
                    ...
                    motion=self.frame.multi_split_panel_ctrl.vmd_file_ctrl.data.copy(), \
            ...
            logger.critical("多段分割処理が意図せぬエラーで終了しました。", e, decoration=MLogger.DECORATION_BOX)
        finally:
            try:
                logger.debug("★★★result: %s, is_killed: %s", self.result, self.is_killed)
                if self.is_out_log or (not self.result and not self.is_killed):
                    # ログパス生成
                    output_vmd_path = self.frame.multi_split_panel_ctrl.output_vmd_file_ctrl.file_ctrl.GetPath()
                    output_log_path = re.sub(r'\.vmd$', '.log', output_vmd_path)

                    # 出力されたメッセージを全部出力
                    self.frame.multi_split_panel_ctrl.console_ctrl.SaveFile(filename=output_log_path)
