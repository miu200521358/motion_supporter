...
class MultiSplitPanel(BasePanel):
    ...
    def __init__(self, frame: wx.Frame, multi_split: wx.Notebook, tab_idx: int):
        ...
        # ゆらぎの大きさ
        self.multi_split_size_txt = wx.StaticText(self, wx.ID_ANY, u"ゆらぎの大きさ", wx.DefaultPosition, wx.DefaultSize, 0)
        self.setting_sizer.Add(self.multi_split_size_txt, 0, wx.ALL, 5)

        self.multi_split_size_ctrl = wx.SpinCtrl(self, id=wx.ID_ANY, size=wx.Size(60, -1), value="8", min=0, max=99999999, initial=8)
        self.multi_split_size_ctrl.SetToolTip(u"ゆらぎの大きさを指定して下さい。値が大きいほど、ゆらぎが大きくなります。")
        self.multi_split_size_ctrl.Bind(wx.EVT_SPINCTRL, self.on_change_file)
        self.setting_sizer.Add(self.multi_split_size_ctrl, 0, wx.ALL, 5)
        ...
        # 実行ボタン
        self.multi_split_btn_ctrl = wx.Button(self, wx.ID_ANY, u"ゆらぎ複製", wx.DefaultPosition, wx.Size(200, 50), 0)
        self.multi_split_btn_ctrl.SetToolTip(u"ゆらぎを付与したモーションを複製します")
    ...
    def set_output_vmd_path(self, event, is_force=False):
        output_multi_split_vmd_path = MFileUtils.get_output_multi_split_vmd_path(
            ...
            self.output_multi_split_vmd_file_ctrl.file_ctrl.GetPath(),
            self.multi_split_size_ctrl.GetValue(), is_force)
