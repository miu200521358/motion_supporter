...
class ArmIKtoFKPanel(BasePanel):
    ...
    def __init__(self, frame: wx.Frame, arm_ik2fk: wx.Notebook, tab_idx: int):
        ...
        # 多段分割変換実行ボタン
        self.arm_ik2fk_btn_ctrl = wx.Button(self, wx.ID_ANY, u"腕ＩＫ変換", wx.DefaultPosition, wx.Size(200, 50), 0)
    ...
    def on_convert(self, event: wx.Event):
        ...
        # 腕ＩＫ変換変換開始
        if self.arm_ik2fk_btn_ctrl.GetLabel() == "腕ＩＫ変換停止" and self.convert_arm_ik2fk_worker:
            # フォーム無効化
            self.disable()
            # 停止状態でボタン押下時、停止
            ...
            logger.warning("腕ＩＫ変換を中断します。", decoration=MLogger.DECORATION_BOX)
            ...
            self.console_ctrl.Clear()
            # ラベル変更
            self.arm_ik2fk_btn_ctrl.SetLabel("腕ＩＫ変換停止")
            self.arm_ik2fk_btn_ctrl.Enable()
